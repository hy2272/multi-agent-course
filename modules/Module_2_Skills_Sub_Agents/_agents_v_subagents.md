Sub Agents vs Agents

You've already used Skills. You know how to give Claude a reusable workflow and invoke it with a slash command. That's one agent, doing one thing, in sequence.
...
![alt text](image.png)
Why isolated Context window matters
...
This is also why subagents are better than just writing a very long prompt. The problem with very long prompts isn't just length — it's that everything is mixed together. Subagents let you keep concerns separated by design.
...
Prompt drift: each subagent's system prompt is its entire understanding of its job. Vague prompts produce vague results. Fix: write subagent prompts the way you'd write

acceptance criteria — specific, testable, scoped.

The Mental Shift

Here's the practical reframe for product managers:

Before multi-agent systems, your job was to write good specs for engineers. You described what to build, they built it, you reviewed it.

With multi-agent systems, your job is to design the agent team. You decide what each agent's role is, what inputs it gets, what outputs it produces, and how the results connect. That's an organizational design problem — the same judgment you already apply to cross-functional teams, now applied to AI agents. The agents are fast and don't need standups. But they still need clear ownership, clear handoffs, and a spec that tells them what "done" looks like. That's your job.
